...
data_dir = '/data/'
Dataset_names_path = "model_data/char_names.txt"
Dataset_train = "model_data/char_train.txt"
Dataset_test = "model_data/char_test.txt"
is_subfolder = False
...
def ParseXML(img_folder, file):
    for img in os.listdir(img_folder):
        ...
        if temp[1] == '.xml':
            ...
            xml_file = img_folder + '/' + temp[0] + '.xml'
            if not os.path.exists(xml_file):
                ...
                for i, obj in enumerate(root.iter('object')):
                    difficult = obj.find('difficult').text
                    cls = obj.find('name').text
                    if not cls.isalpha() or not cls.isalnum():
                        tmp = img_folder
                        print(tmp)
                    if cls not in Dataset_names:
                        Dataset_names.append(cls)
